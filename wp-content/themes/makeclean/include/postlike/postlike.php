...
function sl_enqueue_scripts() {
	...
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		...
		'like' => esc_html__( 'Like', 'makeclean' ),
		'unlike' => esc_html__( 'Unlike', 'makeclean' )
	) ); 
}
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'makeclean' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'makeclean' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'makeclean' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( 'Like', 'makeclean' );
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php esc_html_e( 'You Like:', 'makeclean' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html_e( 'You do not like anything yet.', 'makeclean' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
